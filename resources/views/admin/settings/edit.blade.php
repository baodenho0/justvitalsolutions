            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Edit Setting: {{ $setting->key }}</h3>
                    </div>
                    <form action="{{ route('admin.settings.update', $setting) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @error('type') is-invalid @enderror" id="type" name="type" required>
                                    @foreach($types as $value => $label)
                                        <option value="{{ $value }}" {{ old('type', $setting->type) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                                @error('type')
                                    <span class="invalid-feedback">{{ $message }}</span>
                            ...
                            <div class="form-group" id="textarea-container" style="display: none;">
                                ...
                                <textarea class="form-control @error('value') is-invalid @enderror" id="textarea_value" name="textarea_value" rows="5">{{ old('textarea_value', $setting->value) }}</textarea>
                                @error('value')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group" id="boolean-container" style="display: none;">
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" class="custom-control-input" id="boolean_value" name="boolean_value" value="1" {{ old('boolean_value', $setting->value) ? 'checked' : '' }}>
                            ...
                            <div class="form-group" id="color-container" style="display: none;">
                                ...
                                <input type="color" class="form-control @error('value') is-invalid @enderror" id="color_value" name="color_value" value="{{ old('color_value', $setting->value) }}">
                                @error('value')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group" id="json-container" style="display: none;">
                                ...
                                <textarea class="form-control @error('value') is-invalid @enderror" id="json_value" name="json_value" rows="5">{{ old('json_value', $setting->value) }}</textarea>
                        ...
                        <div class="card-footer">
                            ...
                            <a href="{{ route('admin.settings.index', ['group' => $setting->group]) }}" class="btn btn-default">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
