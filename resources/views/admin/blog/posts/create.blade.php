    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.blog.posts.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group" id="published_at_group" style="{{ old('is_published') ? '' : 'display: none;' }}">
                    <label for="published_at">Publication Date</label>
                    <input type="datetime-local" class="form-control @error('published_at') is-invalid @enderror" id="published_at" name="published_at" value="{{ old('published_at') ? date('Y-m-d\TH:i', strtotime(old('published_at'))) : date('Y-m-d\TH:i') }}">
                    @error('published_at')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
