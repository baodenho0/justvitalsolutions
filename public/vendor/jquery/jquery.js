...
var access = function( elems, fn, key, value, chainable, emptyGet, raw ) {
	...
	if ( toType( key ) === "object" ) {
		chainable = true;
		for ( i in key ) {
			access( elems, fn, i, key[ i ], emptyGet, raw );
		}
...
function boxModelAdjustment( elem, dimension, box, isBorderBox, styles, computedVal ) {
	...
	for ( ; i < 4; i += 2 ) {
		...
		if ( box === "margin" ) {
			delta += jQuery.css( elem, box + cssExpand[ i ], styles );
		}
		...
		if ( !isBorderBox ) {
			...
			// Add padding
			delta += jQuery.css( elem, "padding" + cssExpand[ i ], styles );

			// For "border" or "margin", add border
			if ( box !== "padding" ) {
				delta += jQuery.css( elem, "border" + cssExpand[ i ] + "Width", styles );

			// But still keep track of it otherwise
			} else {
				extra += jQuery.css( elem, "border" + cssExpand[ i ] + "Width", styles );
			}
...
jQuery.each( [ "toggle", "show", "hide" ], function( _i, name ) {
	...
	jQuery.fn[ name ] = function( speed, easing, callback ) {
		return speed == null || typeof speed === "boolean" ?
			...
			this.animate( genFx( name ), speed, easing, callback );
	};
} );
	...
	position: function() {
		...
		if ( jQuery.css( elem, "position" ) === "fixed" ) {
			...
			if ( offsetParent && offsetParent !== elem && offsetParent.nodeType === 1 ) {
				...
				parentOffset.top += jQuery.css( offsetParent, "borderTopWidth" );
